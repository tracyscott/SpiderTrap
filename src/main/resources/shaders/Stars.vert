...
// fire? palette
//[[0.888 0.437 0.132] [0.098 0.857 0.231] [1.438 0.488 1.218] [0.578 1.400 4.977]]
vec3 firepalette(float t) {
    vec3 a = vec3(0.888, 0.437, 0.132);
    vec3 b = vec3(0.098, 0.857, 0.231);
    vec3 c = vec3(1.438, 0.488, 1.218);
    vec3 d = vec3(0.578, 1.400, 4.977);
    return palette(t, a, b, c, d);
}

float ripple(vec2 p, float freq, float rscale) {
...
void main(){
    ...
    vec2 uv1 = uv;
    float rawd = length(uv1);
    uv1 *= rotationMatrix(fTime * 10. - 10. * sweep * rawd);
    float d = length(uv1);
    float m = 0.002*brt/d;
    float bright = clamp(m, 0., 1.);
    float rays = 1. - abs(uv1.x*uv1.y*rsize);
    bright += clamp(rays, 0., 1.);
    col += firepalette(1.-rawd) * bright;

    vec2 uv2 = uv;
    rawd = length(uv2);
    uv2 *= rotationMatrix(fTime * 10. + 30.0 - 10. * sweep * rawd);
    d = length(uv2);
    m = 0.002*brt/d;
    bright = clamp(m, 0., 1.);
    rays = 1. - abs(uv2.x*uv2.y*rsize);
    bright += clamp(rays, 0., 1.);
    col += firepalette(1.-rawd) * bright;

    vec2 uv3 = uv;
    rawd = length(uv3);
    uv3 *= rotationMatrix(fTime * 10. + 60.0 - 10. * sweep * rawd);
    d = length(uv3);
    m = 0.002*brt/d;
    bright += clamp(m, 0., 1.);
    rays = 1. - abs(uv3.x*uv3.y*rsize);
    bright += clamp(rays, 0., 1.);
    col += firepalette(1.-rawd) * bright;

    float center = length(uv);
    col *= smoothstep(0.0, 0.5, center);
    //col -= ripple;
