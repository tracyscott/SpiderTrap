...
float explosion(vec2 uv, float t) {
    ...
    for (float i=0.; i < parts; i++) {
        ...
        float brightness = mix(0.0005, .003, smoothstep(.05, 0., t));
        brightness *= .5 + .5 * sin(t*20. + i);
        brightness *= smoothstep(1., .4, t);
        sparks += pow(brightness/d, fall);
    }
    return sparks;
}
