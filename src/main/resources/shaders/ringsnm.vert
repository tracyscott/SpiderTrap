...
void main(){
    ...
    float distance = 0.;
    {
        ...
        p1.z = p1.z + zOffset;
        p1 = p1 / fScale;
        //p1 = p1 * rotateZ(fTime * s1);
        //p1.y -= 0.05;
        vec2 xz = vec2(p1.x, p1.z);
        //distance = fill(circleSDF(p1), .65);
        float dr = .15 + r * (.5 * sin(fTime) + .5) * .3; //r + mod(fTime,.35);
        distance = circleSDF2(xz, dr);
        distance = opOnion(distance, h);
        //float distance2 = circleSDF2(xz, dr - 0.2);
        //distance2 = opOnion(distance2, h);
    ...
    /*
    {
        vec3 p2 = pos;
        p2 = p2 / fScale;
        //p2 = p2 * rotateX(M_PI_2);
        p2 = p2 * rotateX(fTime * s2);
        //p2.y -= 0.05;
        distance = min(sdVerticalCapsule(p2, h, r), distance);
    }
    *///
