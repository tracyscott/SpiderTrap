...
public class Sdf2DTex extends LXPattern implements UIDeviceControls<Sdf2DTex> {
  private static final Logger logger = Logger.getLogger(Sdf2D.class.getName());
  public GL3 gl;

  StringParameter scriptName = new StringParameter("scriptName", "Kifs");
  ...
  public Sdf2DTex(LX lx) {
    super(lx);

    //Sdf2D.glDrawable.getContext().makeCurrent();
    //logger.info("Sdf2D.glDrawable.getGL()=" + Sdf2D.glDrawable.getGL());
    //gl = Sdf2D.glDrawable.getGL().getGL3();
    ...
    //spGLCtx = GLUtil.spiderGLInit(glTexture, scriptName.getString());
  ...
  public void onParameterChanged(LXParameter p) {
    ...
    if (p == this.texName) {
      logger.info("texture parameter changed!");
  ...
  @Override
  public void buildDeviceControls(LXStudio.UI ui, UIDevice uiDevice, Sdf2DTex pattern) {
      ...
      public void onToggle(boolean on) {
        if (on) {
          ui.applet.selectInput(
              "Select a file to open:",
              "onOpenTexture",
              new File("target/classes/data/textures", "textures"),
              Sdf2DTex.this
          );
        }
      }
  ...
  public void onOpenTexture(final File openFile) {
    ...
    if (openFile != null) {
      LX lx = getLX();
      String baseFilename = openFile.getName().substring(0, openFile.getName().indexOf('.'));
      logger.info("Loading texture: " + baseFilename);

      lx.engine.addTask(() -> {
        logger.info("Running texture name setting task");
        lx.command.perform(new LXCommand.Parameter.SetString(
            texName,
            baseFilename
        ));
