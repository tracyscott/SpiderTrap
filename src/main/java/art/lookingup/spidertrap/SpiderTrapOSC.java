...
/**
 * SpiderTrap specific class for OSC interaction.
 */
public class SpiderTrapOSC implements LXOscListener {
    private static final Logger logger = Logger.getLogger(SpiderTrapOSC.class.getName());

    private static final int OSC_BUFFER_SIZE = 1024;
    public static int OSC_PORT = 7979;
    public LX lx;
    public LXOscEngine.Transmitter ntreeOscTransmitter;

    static public List<String> pendingMessages = new ArrayList<String>();

    public SpiderTrapOSC(LX lx) {
        this.lx = lx;
    ...
    /**
     * Starts an OSC Transmitter for NTree control data with the specified destination address
     * and destination port.  Initiated by a client invoking OSC method /ntree/registerclient with
     * a string argument in the form of hostname:port.  Currently, only one OSC transmitter is supported.
     * Additional calls to /ntree/registerclient will just overwrite any current transmitter.
     *
    ...
    public boolean startOscTransmitter(String address, int port, int bufferSize) {
        try {
            ntreeOscTransmitter = lx.engine.osc.transmitter(InetAddress.getByName(address), port, bufferSize);
    ...
    /**
     * Handle an OSC message from a client.  Root path of address space is /spidertrap.
     *
        ...
        try {
            ...
            if ("spidertrap".equals(path[1])) {
                if (path.length > 2) {
                    ...
                    if ("txtmsg".equals(path[2])) {
                        // Emulate receiving a text message.
                        String msg = message.getString(0);
                        logger.info("Received OSC txt message: " + msg);
                    }
    ...
    public void sendOscMessage(OscMessage message) {
        try {
            if (ntreeOscTransmitter != null) ntreeOscTransmitter.send(message);
